...
import org.cytoscape.graph.centralities.api.NetworkStatistics;
import org.cytoscape.graph.centralities.api.NetworkStatsResults;
...
public class NetworkStatisticsImpl implements NetworkStatistics {
	...
	public NetworkStatsResults computeAll(CyNetwork network, boolean directed,
		...
		for (CyNode tempNode : nodeList) {
			...
			closenessMap.put(tempNode, clo.getCloseness(network, nodeDistanceMap.get(tempNode)));
			radialityMap.put(tempNode, rad.getRadiality(network, diameter, nodeDistanceMap.get(tempNode)));
			cCoefficientMap.put(tempNode, cCoefficient.getClusteringCoefficient(network, tempNode, directed));
